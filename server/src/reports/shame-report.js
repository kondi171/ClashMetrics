...
async function generateShameList(res, wars, players) {
  try {
    ...
    const maxMissed =
      playersWithStats.length > 0 ? playersWithStats[0].totalMissed : 0;
    const totalMissedInClan = playersWithStats.reduce(
      (sum, p) => sum + p.totalMissed,
      0,
    );
    ...
    wars.forEach((war, index) => {
      ...
      sheet.getCell(ROW_OFFSET + 1, col).font = {
        ...BOLD_FONT,
        color: { argb: colors.white },
        size: 8,
      };
      sheet.getCell(ROW_OFFSET + 1, col).alignment = {
        ...centerAlignment,
        wrapText: true,
      };
    ...
    playersWithStats.forEach((player, index) => {
      ...
      const bodyBgColor = isPerfectSeason
        ? colors.successGreen
        : index % 2 === 0
          ? colors.rowEven
          : colors.rowOdd;
        ...
        if (res.status === "MISSING") {
          ...
          if (res.missed === 2) {
            ...
            cell.fill = {
              type: "pattern",
              pattern: "solid",
              fgColor: { argb: colors.warningYellow },
            };
            cell.font = {
              ...NORMAL_FONT,
              color: { argb: "FF000000" },
            ...
            cell.font = {
              ...NORMAL_FONT,
              color: { argb: isPerfectSeason ? "FF004400" : colors.grayText },
            };
          }
        }
      });
    ...
    wars.forEach((war, warIdx) => {
      ...
      const actualAttacks = war.myClan.members.reduce(
        (sum, m) => sum + (m.attacks ? m.attacks.length : 0),
        0,
      );
      ...
      [
        ...
        {
          val: missedPerc + "%",
          row: footerStartRow + 3,
          isPerc: true,
          rawPerc: parseFloat(missedPerc),
        },
      ].forEach((item) => {
    ...
    const globalMissedPerc = (
      (globalMissedCount / globalTotalPossible) *
      100
    ).toFixed(1);
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="ShameList.xlsx"',
    );
