...
// Grupowanie rund w bloki sezonów dla nagłówka nadrzędnego[cite: 6]
...
<template>
  <table class="scores-table">
    <thead>
      <tr class="row-season-head" v-if="seasonHeaders.length > 0">
        ...
        <th rowspan="3" class="col-avg">OBRONA</th>
        <th
          v-for="(s, idx) in seasonHeaders"
          :key="'season-h-' + idx"
          :colspan="s.colspan"
          class="col-season-title"
        >
    ...
    <tbody>
      <template v-for="(p, idx) in players" :key="p.tag">
        <tr :class="getPodiumClass(idx)">
          ...
          <td class="cell-total def-total">{{ p.totalDefStars }}</td>
          <td :class="['cell-avg', getAvgClass(p.avgDefStars)]">
            {{ p.avgDefStars }} ★
          </td>
        ...
        >
          ...
          <template v-for="(war, wIdx) in p.wars" :key="'def-dest-' + wIdx">
